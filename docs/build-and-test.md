...
- `make mr-test` 运行 MR table、MR registration manager、MR deregistration manager、MR key checker、MR access checker 和 MR PD checker 测试，覆盖 MR entry 写入、方向敏感的 lkey/rkey lookup、key alias rejection、VA->PA 范围转换、bounds/length 错误、owner function 隔离、refcount inc/dec 上下溢、pending_deregister 拒绝新访问、REGISTER_MR 请求校验、SG entry fetch mock、DEREGISTER_MR refcount drain、clear entry、timeout、本地路径使用 rkey 拒绝、远端路径使用 lkey 拒绝、local/remote/MW access_flags 权限矩阵、QP PD 与 MR PD mismatch 和成功响应。
