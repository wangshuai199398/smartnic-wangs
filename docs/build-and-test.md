...
当前它会进入 `sim/cocotb`，依次调用 PCIe 控制面、Doorbell path、QP manager、CQ manager 和 MR manager 的模块级测试入口。如果本机尚未安装 Cocotb 或 Verilator，对应子目标会打印跳过信息。

它支撑的验证目标是用 Python 测试激励验证 Doorbell、QP、CQ、MR、DMA、RoCEv2 packet 和 completion 等模块行为。

## make pcie-test / doorbell-test / qp-test / cq-test / mr-test
