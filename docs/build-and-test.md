...
- `make dma-test` 运行 DMA descriptor dispatcher、WQE/SGE fetcher 和 SGE traversal 测试，覆盖 SQ Send/RDMA Write host read、RQ Recv host write、RDMA Read response host write、CQE write、unsupported opcode、zero length、output backpressure、fixed-priority arbitration、SQ/RQ WQE fetch 地址计算、WQE decode、inline SGE、extended SGE list fetch、256 SGE 边界、response backpressure、SGE total-length accounting、byte_offset、zero-overlap validation、index 顺序检查和 traversal backpressure。
