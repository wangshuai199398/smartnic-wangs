...
当前它会进入 `sim/cocotb`，依次调用 PCIe 控制面、Doorbell path、QP manager、CQ manager、MR manager 和 DMA engine 的模块级测试入口。如果本机尚未安装 Cocotb 或 Verilator，对应子目标会打印跳过信息。

它支撑的验证目标是用 Python 测试激励验证 Doorbell、QP、CQ、MR、DMA、RoCEv2 packet 和 completion 等模块行为。

## make pcie-test / doorbell-test / qp-test / cq-test / mr-test / dma-test
...
- `make cq-test` 运行 CQ context table、completion engine、CQE write path、CQ index manager、CQ notification 和 CQ integration 测试，覆盖 CQN lookup、alias rejection、CQ arm consumer index 更新、completion producer index 更新、owner function 权限检查、overflow set/clear、SQ/RQ/cleanup/error event 到 64-byte CQE 的格式化、CQE host 地址计算、64-byte DMA write 请求、reserved-slot full/empty 判断、producer/consumer wraparound、CQ arm race、polling mode、solicited-only、interrupt moderation count/timer、MSI-X request backpressure，以及 mock completion path 的最小端到端串联。
- `make mr-test` 运行 MR table、MR registration manager、MR deregistration manager、MR key checker、MR access checker、MR PD checker、Memory Window manager 和 MR integration 测试，覆盖 MR entry 写入、方向敏感的 lkey/rkey lookup、key alias rejection、VA->PA 范围转换、bounds/length 错误、owner function 隔离、refcount inc/dec 上下溢、pending_deregister 拒绝新访问、REGISTER_MR 请求校验、SG entry fetch mock、DEREGISTER_MR refcount drain、clear entry、timeout、本地路径使用 rkey 拒绝、远端路径使用 lkey 拒绝、local/remote/MW access_flags 权限矩阵、QP PD 与 MR PD mismatch、MW bind/unbind、权限子集、QP error invalidation、成功响应，以及 REGISTER_MR 到 DEREGISTER_MR、parent MR 到 MW unbind 的最小端到端保护路径。
- `make dma-test` 运行 DMA descriptor dispatcher、WQE/SGE fetcher、SGE traversal、DMA/MR integration、segment splitter、DMA arbiter、host read path、host write path 和 DMA error propagation 测试，覆盖 SQ Send/RDMA Write host read、RQ Recv host write、RDMA Read response host write、CQE write、unsupported opcode、zero length、output backpressure、fixed-priority arbitration、SQ/RQ WQE fetch 地址计算、WQE decode、inline SGE、extended SGE list fetch、256 SGE 边界、response backpressure、SGE total-length accounting、byte_offset、zero-overlap validation、index 顺序检查、traversal backpressure、每个 DMA segment 的 lkey/rkey 方向检查、access_flags、PD、bounds、Memory Window 状态、refcount increment、protected segment backpressure、PMTU/4KB/max segment split、split sub_index 和 last 标志、fixed priority/round-robin/weighted round-robin/starvation guard 仲裁、grant backpressure、protected segment 到 PCIe read request、read response 到 transport payload stream、tag/length/error 检查、read 完成后的 refcount release、protected segment 加 payload 到 PCIe write request、write completion 到 write_done、write backpressure、write 错误路径、write 完成后的 refcount release、DMA error 到 completion status 的映射、fatal QP error request、completion backpressure 和多 error source 优先级。
