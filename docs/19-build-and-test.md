...
- `make dma-test` 运行 DMA descriptor dispatcher、WQE/SGE fetcher、SGE traversal、DMA/MR integration、segment splitter、host read path 和 host write path 测试，覆盖 SQ Send/RDMA Write host read、RQ Recv host write、RDMA Read response host write、CQE write、unsupported opcode、zero length、output backpressure、fixed-priority arbitration、SQ/RQ WQE fetch 地址计算、WQE decode、inline SGE、extended SGE list fetch、256 SGE 边界、response backpressure、SGE total-length accounting、byte_offset、zero-overlap validation、index 顺序检查、traversal backpressure、每个 DMA segment 的 lkey/rkey 方向检查、access_flags、PD、bounds、Memory Window 状态、refcount increment、protected segment backpressure、PMTU/4KB/max segment split、split sub_index 和 last 标志、protected segment 到 PCIe read request、read response 到 transport payload stream、tag/length/error 检查、read 完成后的 refcount release、protected segment 加 payload 到 PCIe write request、write completion 到 write_done、write backpressure、write 错误路径和 write 完成后的 refcount release。
